# -*- coding: utf-8 -*-
"""Final_Optimization.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ssrcl5qULGZsWnbmWCcmLj_Z_johv0dw
"""
...
url = 'https://raw.githubusercontent.com/Dadoyen2/Geometry-Optimization/main/pinane.mol2'
...
def calculate_torsions_with_gradient(atoms, torsion_params):
    coords = [np.array(atom[1:]) for atom in atoms]  # Ensure coordinates are numpy arrays
    n_atoms = len(atoms)
    torsions = []
    dihedrals = []
...
#torsion_gradients = calculate_torsions_with_gradient(atoms, torsion_params)

Total_energy_vanderwaals = []
def calculate_vdw_energy_with_gradient(atoms, bonds_new, angles_new):
    # Define epsilon and sigma values for Hydrogen and Carbon
    epsilon_values = {'H': 0.03, 'C': 0.07}  # in kcal/mol
    sigma_values = {'H': 1.20, 'C': 1.75}    # in Angstroms
...
#van_der_waal_gradients = calculate_vdw_energy_with_gradient(atoms, bonds_new, angles_new)
    ...
    with open(os.devnull, 'w') as fnull:
        with contextlib.redirect_stdout(fnull):
            try:
                ...
                torsion_energy = calculate_torsions_with_gradient(atoms, torsion_params)
                van_der_waal_gradients = calculate_vdw_energy_with_gradient(atoms, bonds_new, angles_new)
...
def optimization_cycle(atoms, M_k, max_cycles=1000):
